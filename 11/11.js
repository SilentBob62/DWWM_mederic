var divVisit=document.getElementById("visit");
var init=document.getElementById("init");
init.addEventListener("click",initialise);

if (readCookie("init")==null){
   createCookie("init",1,5);
}
else{
    createCookie("init", parseInt(readCookie("init"))+1,5);
}

divVisit.innerHTML="Cette page a été visité "+readCookie("init")+" fois";

function initialise(){
    createCookie("init",0,5);
    // location.reload();
}
